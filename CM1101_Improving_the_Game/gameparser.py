...
def normalise_input(user_input):
    ...
    # Remove whitespace 
    space_free_text = no_punct.strip()

    # Create a list of words
    words = []
    word = ""
    for char in space_free_text:
        if char != " ":
           word = word + char
        elif char == " " and word == "":
            word = ""
            ...
            words.append(word)
            word = ""
    words.append(word) #this adds the final word (no space after it)

    # Filter out unimportant words
    words_filtered = filter_words(words, skip_words)
