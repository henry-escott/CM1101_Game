...
def remove_punct(text):
    ...
    for char in text:
       if char not in punct:
           punct_free_text = punct_free_text + char
    return punct_free_text
...
def remove_spaces(text):
    ...
    #space_free_text = ''.join(text.split())
    space_free_text = text.strip()
...
def normalise_input(user_input):
    ...
    normalized_input = remove_punct(normalized_input)
    normalized_input = remove_spaces(normalized_input)
...
def menu(exits):
    ...
    while True:
        ...
        user_input = input()
        # Normalise the input
        normalized_input = normalise_input(user_input)
        # Check if the input makes sense (is valid exit)
        if is_valid_exit(exits, normalized_input) == True:
